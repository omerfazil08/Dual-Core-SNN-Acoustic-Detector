...
SOURCE_ROOT = 'C:/Users/omer-/Desktop/drone_audio_detector/drone_analysis_data'
DEST_ROOT = 'C:/Users/omer-/Desktop/drone_audio_detector/drone_analysis_data/drone_subsets'
TARGET_SR = 16000
RANDOM_SEED = 42
...
train_bebop_path = os.path.join(SOURCE_ROOT, 'train', 'bebop', '*.wav')
train_unknown_path = os.path.join(SOURCE_ROOT, 'train', 'unknown', '*.wav')

train_bebop_files = sorted(glob.glob(train_bebop_path))
train_noise_files = sorted(glob.glob(train_unknown_path))

print(f"Found {len(train_bebop_files)} Bebop files.")
print(f"Found {len(train_noise_files)} Unknown files.")

if len(train_bebop_files) == 0:
    print("❌ ERROR: No Bebop files found. Check SOURCE_ROOT path.")
    exit()
...
bebop_freqs = {}
for i, f in enumerate(train_bebop_files):
    if i % 100 == 0:
        print(f"  Processing Bebop file {i+1}/{len(train_bebop_files)}...")
    freq = get_fundamental_freq(f)
    if freq:
        bebop_freqs[f] = freq

print(f"✅ Successfully analyzed {len(bebop_freqs)} Bebop files.")
...
# --- SELECT REPRESENTATIVE BEBOP FILES ---
avg_bpf = np.median(list(bebop_freqs.values()))
print(f"\n📊 Average Bebop BPF: {avg_bpf:.1f} Hz")
sorted_bebop = sorted(bebop_freqs.items(), key=lambda item: abs(item[1] - avg_bpf))
...
for name, count in subsets.items():
    ...
    # Select Bebop files
    selected_bebop = [f for f, _ in sorted_bebop[:count]]
    for src in selected_bebop:
    ...
    print(f"   → Copied {len(selected_bebop)} drone and {len(selected_noise)} noise files.")
